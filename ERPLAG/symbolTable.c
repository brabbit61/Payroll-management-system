...
/* returns flag = 0 when the input is taken by the user or when input is assigned if there is no  input specified then a error occurs in the calling function */
...
void checkFor(IDEntry* ctrl_var, tokenInfo* token, hashTable2 tableID, int print)
{
	...
	while(token != NULL && token->s != END)
	{
		...
		// if the ID is an array then flag=1
		...
		while(token != NULL && token->s != SEMICOL)
		{
			...
			{
				//defines whether the value of the variable can be changed or no
				if(print == 1)
				{
					printf("\nERROR : The for statement must not redefine the variable \"%s\" at line number %d", ctrl_var->entity.ifunc.word, token->lno);
				}
				...
				break;//stop scanning the for loop
			}
			token = token->n;
		}
	}
...
void createIDTable(tokenInfo* token, totalScopeList* scope_tot, hashTable2 tableID, hashTable2 tableFunc, int print)
{
	...
	while(token != NULL)
	{
			...
			while(token != NULL && token->s != SQBC)
			{
				...
				else
				{
					...
					{	
						...
						res = findScope(tableFunc,current_function, TREE->scope, 1);
						//create link list of parameter values that are returned
		...
		else if(token->s == DECLARE)
		{
			...
			if(token->n->s != MODULE)
			{
				token = token->n;//points to the first variable that is declared
				IDEntry* par_data = (IDEntry*)malloc(sizeof(IDEntry));
				strcpy(par_data->entity.ivar.word, token->val.s);
				par_data->entity.ivar.code = id_counter++;
				par_data->entity.ivar.lno = token->lno;
				...
				else
				{
					...
					par_data->entity.ivar.offset = function_offset;
					addScope(tableID, par_data, 0);//adding the first variable to the table
					int bytes = par_data->entity.ivar.bytes;
					if (par_data->entity.ivar.v_a == 1) 
					{
						bytes *= (par_data->entity.ivar.var.a.e_idx - par_data->entity.ivar.var.a.s_idx + 1);
				...
				//adding the remaining variables to the table
		...
		//checks if the varible being used is already defined or not
		else if(token->s == ID)
		{
